function bIs = isFile_SUESIMat( cFile )
% bIs = isFile_SUESIMat( cFile )
%
% Utility to determine if the given file(s) is/are PARSED SUESI .mat files
%
% Params:
...
for iFile = 1:numel(cFile)
    if isfile( cFile{iFile} )
        try %#ok<TRYNC>
            % NB: Only supports the .mat files created by decodeSUESI not the
            % files created by the now deprecated parseSuesiLog.m
            m = matfile( cFile{iFile} );
            if numel( who( m, 'nSyncRange', 'nData', 'col', 'tblWaveForm' ) ) == 4
                bIs(iFile) = ~isempty( m.nSyncRange );
            end
        end
    end
end
