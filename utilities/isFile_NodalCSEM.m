function bIs = isFile_NodalCSEM( cFile )
% bIs = isFile_NodalCSEM( cFile )
%
% Utility to determine if the given file(s) is/are nodal CSEM files
%
% Params:
...
for iFile = 1:numel(cFile)
    if isfile( cFile{iFile} )
        try %#ok<TRYNC>
            ...
            if numel( who( m, 'stRx', 'nTF', 'nVar', 'tmWind', 'tNav' ) ) == 5
                % New format from cwave
                bIs(iFile) = true;
            elseif numel( who( m, 'stCSEM' ) ) == 1
                % This is the old format produced by my "csem*.m" codes
