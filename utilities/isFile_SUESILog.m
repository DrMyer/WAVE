function bIs = isFile_SUESILog( cFile )
% bIs = isFile_SUESILog( cFile )
%
% Utility to determine if the given file(s) is/are SUESI log files
%
% Params:
...
for iFile = 1:numel(cFile)
    if isfile( cFile{iFile} )
        ...
        if fid > -1
            % SUESI logs are somewhat freeform. So look for something
            % recognizable in the first so many lines
            for iLine = 1:100
                s = fgetl( fid );
                if strncmpi( s, 'S= ', 3 ) ...
                || strncmpi( s, 'SUESI 500', 9 ) ...
                || strncmpi( s, '-- SUESI 500', 12 ) ...
                || strncmpi( s, '-- SYSTEM RESETTING  ABCDEF--', 29 )
                    bIs(iFile) = true;
                    break;
                end
                if feof(fid)
