function NodalCSEM( oWave, sScope )
% Calculate & stack CSEM data for nodal receivers
%
% Params:
...
%-------------------------------------------------------------------------------
    ...
    % to process CSEM as data are recovered from the seafloor then just use "Run
    % New" each time they complete another site
    bChkRx = cwave.ValidateRxCfg( oWave.tableRxCfg, oWave.tableRxCh, oWave.sDir_Calib );
    ...
    % start/end times overlap with the data files operation times
    ...
    oProg = uiprogressdlg( oWave.hFig, 'Title', 'Nodal CSEM', 'Cancelable', true ...
        , 'Message', 'Processing Nodal CSEM ...' );
    cMsgText = cell(height(tProc),1);
    nMsgType = zeros(height(tProc),1) + cwave.LogOK;
    tStart   = tic();
    for iProc = 1:height(tProc)
        ...
        oProg.Message   = {'Processing Nodal CSEM ...';sRemain};
        
        % Process one Rx + Tow
        [nMsgType(iProc),cMsgText{iProc}] = sub_CSEM( tProc(iProc,:) ...
            , oWave.nWindowLen, oWave.nStackLen ...
            , oWave.tableHarmonics, oWave.tableSDM, oWave.tableTxNav );
        
    end % to-be-processed list
    close( oProg );
    
    % Dump all the messages into the log
    cOutList = {};
    for i = 1:numel(cMsgText)
        if ~isempty(cMsgText{i})
            oWave.AddLog( nMsgType(i), cwave.sLog_Nodal_CSEM, cMsgText{i} );
        end
        if nMsgType(i) == cwave.LogOK
            cOutList{end+1,1} = char(tProc.OutFile(i));
        end
    end
    
    % Update the output file list. This will fire off any listeners
    if bAll || isempty( oWave.cFiles_NodalCSEM )
        oWave.cFiles_NodalCSEM = sort( cOutList );
    else
        oWave.cFiles_NodalCSEM = unique( [oWave.cFiles_NodalCSEM; cOutList] );
    end
    
    % Finally, tell how long it took
    nDur = seconds( toc(tStart) );
    nDur.Format = 'hh:mm:ss';
    oWave.AddLog( cwave.LogOK, cwave.sLog_Nodal_CSEM, ['Process time: ' char(nDur)] );
    
    % Put up a "done" message but only if there are no warnings or errors. In
    % that case w_panelAction will automatically open the log to show them
    if all( nMsgType == cwave.LogOK )
        uialert( oWave.hFig, {
            sprintf( 'Successfully processed %d Rx+Tow Line combinations.', height(tProc) )
            }, 'Calculate Nodal CSEM', 'Icon', 'success' );
    end
...
% done in w_tabNodalCSEM and keep the panel unavailable until they are met.
function bOK = sub_CheckPieces( oWave, bChkRx )
    ...
    if isempty( oWave.tableRxCfg )
        uialert( oWave.hFig, {
            ...
            }, 'Calculate Nodal CSEM' );
        return;
    end
    sList = unique( oWave.tableRxCfg.BinPath(bChkRx) );
    ...
    sList = fullfile( oWave.tableRxCfg.BinPath(bChkRx) ...
                    , oWave.tableRxCfg.BinFile(bChkRx) );
    sList(isfile(sList)) = [];
    if ~isempty( sList )
        uialert( oWave.hFig, [
...
function tProc = sub_BuildProcList( oWave, bAll, bChkRx )
    ...
    tProc = table( 'Size', [0 18] ...
        ...
        , 'VariableTypes', [{'string','double','double'} repmat({'datetime'},1,5) ...
                            repmat({'double'},1,8) repmat({'string'},1,2)] ... 
        );
    ...
    for iRx = 1:height(oWave.tableRxCfg)
        ...
        sInFile = fullfile( oWave.tableRxCfg.BinPath(iRx), oWave.tableRxCfg.BinFile(iRx) );
        try
            stData = readBinData( sInFile );
        catch Me
            cErr{end+1,1} = sprintf( '%s: Error %s' ...
                , oWave.tableRxCfg.RxName(iRx), Me.message );
            continue;
        end
        
        % Does the channel count match that in the user's receiver config?
        bCh = strcmpi( oWave.tableRxCfg.RxName(iRx), oWave.tableRxCh.RxName );
        nChCnt = sum( bCh );
        if stData.nChanCnt ~= nChCnt
            cErr{end+1,1} = sprintf( '%s: has %d channels but %d are configured' ...
                , oWave.tableRxCfg.RxName(iRx), stData.nChanCnt, nChCnt );
            continue;
        end
        
        % Get the list of channel calibration files to apply IN ORDER
        tCh = sortrows( oWave.tableRxCh(bCh,:), 'ChanNo' );
        ...
        for iCh = 1:nChCnt
            ...
            if ~isfile( sCalibFile )
                cErr{end+1,1} = sprintf( '%s: Channel %d''s calibration file not found' ...
                    , oWave.tableRxCfg.RxName(iRx), iCh );
                cCh = {};
                break;
            end
        ...
        for iTow = 1:height(oWave.tableTow)
            ...
            % that requires a re-run, I only clear the cwave.cFiles_NodalCSEM. I
            % don't delete the files on disk (just in case).
            sOutFile = fullfile( oWave.sCSEMDir, sprintf( 'Line%g_%s.csem.mat' ...
                , oWave.tableTow.TowNo(iTow), oWave.tableRxCfg.RxName(iRx) ) );
            if ~bAll && ismember( sOutFile, oWave.cFiles_NodalCSEM )
                continue;
            end
            ...
            tProc{end+1,:}      = missing();
            tProc.RxName(end)   = oWave.tableRxCfg.RxName(iRx);
            ...
            tProc.SyncTime(end) = oWave.tableRxCfg.SyncTime(iRx);
            tProc.DriftRate(end)= oWave.tableRxCfg.DriftRate(iRx);
            tProc.Latitude(end) = oWave.tableRxCfg.Latitude(iRx);
            tProc.Longitude(end)= oWave.tableRxCfg.Longitude(iRx);
            tProc.Depth(end)    = oWave.tableRxCfg.Depth(iRx);
            tProc.East(end)     = oWave.tableRxCfg.East(iRx);
            tProc.North(end)    = oWave.tableRxCfg.North(iRx);
...
%
function [nMsgType,sMsgText] = sub_CSEM( tProc, nWindSec, nStackSec, tHarm, tSDM, tTxNav )
    ...
    tmWind = mean( tmWind ).';  % center time of each window
    stData.nData = [];
    
    %% Rotate to in/crossline for this tow
    ...
    [iHx,iHy] = sub_Rot( 'Hx', 'Hy', 1 );   % allow for there to be multiple copies of channels
    sub_Rot( 'Hx', 'Hy', 2 );
    ...
    bDrop = ~btwn( tProc.TowFrom, tTxNav.Time, tProc.TowTo );
    tTxNav(bDrop,:) = [];
    ...
    tNav = array2table( interp1( tTxNav.Time, nNav, reshape(tmWind,[],1), 'linear', 'extrap' ) ...
        , 'VariableNames' ...
        , {'AlongTrack', 'Altitude', 'Depth', 'COG', 'Dip', 'East', 'North', 'Lon', 'Lat'} );
    
    %% Calc in/out range for just the segment navigated for this RX
    [tNav.Range, tNav.OffsetAngle] = sub_CalcRange( tNav );
    
    %% Accrue header info
    % Include both useful and "here's what I used" historical info in the header
    stRx = table2struct( tProc(1,{'RxName','TowNo','Latitude','Longitude' ...
        ,'Depth','East','North','InFile'}) );
    stRx.nOrientX   = tProc.DirEofN;
    stRx.iHx        = iHx;  % indices of often sought channels, for convenience
    stRx.iHy        = iHy;
    stRx.iEx        = iEx;
    stRx.iEy        = iEy;
    ...
    save( tProc.OutFile, 'stRx', 'nTF', 'nVar', 'tmWind', 'tNav', 'tCh', '-v7.3' );
    
    % Create the "success" output message to go in the log
    nMsgType = cwave.LogOK;
    sMsgText = sprintf( '%s Line %d saved to %s', stRx.RxName, stRx.TowNo, tProc.OutFile );
    
    return;
    
    %---------------------------------------------------------------------------
    % Rotate the Nth occurrence of the given X & Y channels
    function [iX,iY] = sub_Rot( sX, sY, iOccur )
        % Ensure the return values exist
        [iX,iY] = deal([]); %#ok<ASGLU>
        
        % Find the requested occurrence of each channel in the pair
        iX = find( strcmpi( tCh.Type, sX ) );
        if iOccur > numel(iX)
            return;
        end
        iX = iX(iOccur);
        iY = find( strcmpi( tCh.Type, sY ) );
        if iOccur > numel(iY)
            return;
        end
        iY = iY(iOccur);
        
        % Account for non-orthogonality
        nOrientX = tCh.Orient(iX);
        nOrientY = tCh.Orient(iY);
        if nOrientX > nOrientY
            nHalfDiff = (90 - ((360 - nOrientX) + nOrientY)) / 2;
        else
            nHalfDiff = (90 - (nOrientY - nOrientX)) / 2;
        end
        nOrientX = nOrientX - nHalfDiff;    % sign(nHalfDiff) is important here
        nOrientY = nOrientY + nHalfDiff;
        nHalfDiff = abs(nHalfDiff);         % no longer care about the sign
        
        % Assume instrument EX is North and EY is East
        % Also assume all angles are compass angles so rotate clockwise
        nRotAngle = tProc.DirEofN - nOrientX;
        nRot = [ cosd(nRotAngle)  sind(nRotAngle)
                -sind(nRotAngle)  cosd(nRotAngle)
               ] * cosd(nHalfDiff);
        
        % Rotate the transfer functions & variances
        % nTF = dim( time, channel, freq );
        for iFreq = 1:size(nTF,3)
            nTF(:,[iX iY],iFreq)  = (nRot * squeeze(nTF(:,[iX iY],iFreq)).').';
            nVar(:,[iX iY],iFreq) = (nRot * squeeze(nVar(:,[iX iY],iFreq)).').';
        end
        
        % Rotate the channel Tilts
        n = nRot * tCh.Tilt([iX iY]);
        tCh.Tilt(iX) = n(1);
        tCh.Tilt(iY) = n(2);
        
        % Put the new orientations in the channel list
        tCh.Orient(iX) = mod( nOrientX + nRotAngle, 360 );
        tCh.Orient(iY) = mod( nOrientY + nRotAngle, 360 );
        
        return;
    end % sub_Rot
    
    %---------------------------------------------------------------------------
    % Calculate Range for the given nav table against the RX location
    function [nRange,nOffs] = sub_CalcRange( tNav )
        % NB: The defn of "intow" vs "outtow" only has any meaning for a
        % relatively straight line tow. If the tow path is curved, zig-zag, or
        % any other such weirdness, then in/out is meaningless.
        nRange       = sqrt( (tProc.East  - tNav.East).^2 ...
                           + (tProc.North - tNav.North).^2 );
                       
        % Use the offset angle (angle from Rx at origin to Tx) to determine when
        % we've gone from in-tow to out-tow. When the offset angle is > 90
        % degrees different from the tow line orientation, then we're in intow.
        % NB: "90 -" switches from cartesian angle to compass
        nOffs   = 90 - atan2d( tNav.North - tProc.North, tNav.East - tProc.East );
        bInTow  = abs( phaseDiff( nOffs, tProc.DirEofN, 'Degrees' ) ) > 90;
        nRange(bInTow) = -nRange(bInTow);
