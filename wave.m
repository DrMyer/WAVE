...
% warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. To view
% the GNU General Public License see <https://www.gnu.org/licenses/>
%---------------------------------------------------------------------------
%{
Note about development decisions:
    ...
    by learning from your prof's or collaborators' crappy code. It is 100% worth
    your time. Trust me on this.
...
FUNDING
    ...
    Institution of Oceanography, UCSD. Most of the routines were rewritten from
...
%}
    ...
    % Create an instance of the UI object class and return without dumping
    % anything to the command window display.
    if nargin() > 0
        assert( isfile( varargin{1} ), 'WAVE: 1st param must be an existing path+filename.' );
        load( varargin{1} );    % the saved file contains an obj which will build itself
    else
        oWave = cwave();
        oWave.MakeUI();
    end
